...
const ConsumerScreen = () => {
    ...
    const showMore = () => {
        console.log(consumerProducts.length);
        
        if (numberOfItems + 3 <= consumerProducts.length) {
            console.log(`Increasing number of items from ${numberOfItems} to ${numberOfItems + 3} ,${consumerProducts.length}`)
            setNumberOfItems(numberOfItems + 3)
        } else {
            console.log(`Setting number of items to ${consumerProducts.length}`)
    ...
    return (
        <div className="consumerProductScreen">
            <Meta
                title="Agrocraft | Consumer"
            />
            <Container className='consumerContainer'>
                <h1 className="title">CONSUMER</h1>
                <h4 className="consumer-title">
                    No need to visit field to get grains!!! Just order here and and get all kinds of grains deliverd at your doorstep. Why to wait? Go and order.</h4>
                <br />
                {
                    loading ? <Loader />
