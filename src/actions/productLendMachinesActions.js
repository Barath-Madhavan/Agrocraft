...
export const createLendMachine = (data) => async (dispatch, getState) => {
    try {
        ...
        const { data: createdMachine } = await axios.post(`/api/lendMachines`, data, config)

        dispatch({
            type: MACHINE_CREATE_SUCCESS,
            payload: createdMachine
        })
    } catch (error) {
        dispatch({
...
export const updateLendMachine = (machine) => async (dispatch, getState) => {
    try {
        ...
        const { _id, name, type, location } = machine;
        const updatedMachine = { name, type, location };
        ...
        const { data } = await axios.put(`/api/lendMachines/${_id}`, updatedMachine, config)
